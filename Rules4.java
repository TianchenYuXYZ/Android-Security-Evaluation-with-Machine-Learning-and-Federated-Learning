...
public class Rules4 {
    ...
    private void checkBootloader(Facts device) {
        if("bluejay-1.2-8893284".equals(device.getBootloader())){
            device.setBootloaderScore(2);
        }else if("bluejay-1.1-8893284".equals(device.getBootloader())){
    ...
    private void checkMetered(Facts network){
        ...
        if (a.contains("NOT_METERED") || network.getNetwork_results().get("communication Encrypted")=="false"){
    ...
    private void checkTrusted(Facts network){
        String a = network.getNetwork_results().get("active network info");
        if (a.contains("NOT_TRUSTED") || network.getNetwork_results().get("communication Encrypted")=="false"){
